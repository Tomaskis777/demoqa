...
continue_game = False  # Переменная для продолжения игры
...
def hide_second_bubble():
    ...
    third_bubble_appear_time = time.time() + 2  # Пауза перед появлением третьего облачка

def show_third_bubble():
    global third_bubble_displayed, input_active, third_bubble_shown_once
    third_bubble_displayed = True
    input_active = False  # Отключаем ввод текста пользователем
    third_bubble_shown_once = True  # Устанавливаем флаг, что облачко уже было показано
    threading.Timer(2.0, hide_third_bubble).start()  # Таймер для скрытия облачка через 2 секунды
...
def hide_third_bubble():
    ...
    input_active = False
    input_text = ""  # Сбрасываем введенный текст
    guess_number_bubble_appear_time = time.time() + 2  # Пауза перед началом игры "Угадай число"
...
while running:
    ...
    if second_bubble_displayed:
        ...
        if input_text.lower() == "да":
            ...
            running = False
            third_bubble_appear_time = current_time + 1
    ...
    if third_bubble_displayed:
        draw_speech_bubble(screen, (350, 200), "Хотите сыграть в игру 'Угадай число'?", font, (0, 0, 0), (255, 255, 255))
        threading.Timer(1, show_guess_number_bubble).start()  # Пауза перед показом облачка
